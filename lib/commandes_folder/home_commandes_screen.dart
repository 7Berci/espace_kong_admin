...
Future<void> duplicateCollection(String source, String target) async {
  final firestore = FirebaseFirestore.instance;

  final snapshot = await firestore.collection(source).get();

  for (final doc in snapshot.docs) {
    await firestore.collection(target).doc(doc.id).set(doc.data());
  }

  print("Duplication terminée de '$source' vers '$target'.");
}
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Center(
          child: ElevatedButton(
            onPressed: () => duplicateCollection('products', 'products_iron'),
            child: Text('Dupliquer les collections'),
          ),
        ),
      ],
    );
